...
pub fn generate_action_files(
    ...
    GeneratedActionOutput {
        files: vec![
            client_file,
            render_server_file(generated_dir, actions, preserve_generated_comments),
        ],
        diagnostics,
    }
}
...
mod tests {
    ...
    fn renders_deterministic_generated_files() {
        ...
        let output = generate_action_files("src/.aruna", &actions, true);
    ...
    fn renders_supported_schema_metadata_to_typescript() {
        ...
        let output = generate_action_files("src/.aruna", &[action], true);
